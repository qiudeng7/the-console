...
/**
 * 开发环境数据库配置，使用本地sqlite
 * 
 * 命令:
 * 1. 创建migrations: `pnpm makemigrations` 或 `drizzle-kit generate --config server/utils/drizzle/config-dev.ts`
 * 2. 创建开发环境sqlite数据库: `pnpm migrate` 或 `drizzle-kit migrate --config server/utils/drizzle/config-dev.ts`
 * 
 * 配置:
 * 1. 下面的 `databaseDir`
 * 
...
export const databaseDir = "server/database"
