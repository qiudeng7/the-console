...
describe('Authentication E2E Tests', () => {
  ...
  describe('User Registration', () => {
    it('should register a new user successfully', async () => {
      ...
      await crawler.run({
        requestQueue: [`${BASE_URL}/register`],
        requestHandler: async ({ page, enqueueLinks }) => {
          ...
          const hasSuccess = await page.locator('text=/成功|欢迎|dashboard/i').count() > 0
  ...
  describe('Admin Task Management', () => {
    ...
    it('should create a new task', async () => {
      await crawler.run({
        requestQueue: [`${BASE_URL}/admin/tasks`],
        requestHandler: async ({ page }) => {
          // Click create button
  ...
  describe('Employee Task View', () => {
    const testEmail = `employee-e2e-${Date.now()}@example.com`
    ...
    it('should view assigned tasks', async () => {
      await crawler.run({
        requestQueue: [`${BASE_URL}/employee/tasks`],
        requestHandler: async ({ page }) => {
          // Check page title
