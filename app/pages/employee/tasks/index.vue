...
<template>
  <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
      <h1 class="text-2xl font-bold text-gray-900 mb-6">我的任务</h1>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
          ...
          >
            ...
            <div class="flex items-center text-xs text-gray-400">
              <span v-if="task.category" class="bg-gray-100 px-2 py-1 rounded mr-2">
                {{ task.category }}
              </span>
      ...
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <h3 class="text-lg font-medium text-gray-900 mb-4">{{ selectedTask.title }}</h3>
            <div class="space-y-3">
              ...
              <div v-if="selectedTask.category" class="flex items-center">
                ...
                <span class="text-sm text-gray-900">{{ selectedTask.category }}</span>
              </div>
              <div v-if="selectedTask.tag" class="flex items-center">
                <span class="text-sm font-medium text-gray-500 w-20">标签：</span>
                <span class="text-sm text-gray-900">{{ selectedTask.tag }}</span>
              </div>
              <div class="flex items-center">
                <span class="text-sm font-medium text-gray-500 w-20">创建时间：</span>
                <span class="text-sm text-gray-900">{{ selectedTask.createdAt }}</span>
              </div>
              <div v-if="selectedTask.description" class="mt-4">
                <span class="text-sm font-medium text-gray-500">描述：</span>
                <p class="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{{ selectedTask.description }}</p>
              </div>
            </div>
          </div>
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button type="button" @click="showDetailModal = false" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm">
              关闭
            </button>
          </div>
        </div>
